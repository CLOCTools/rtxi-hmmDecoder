...
extern "C" Plugin::Object *createRTXIPlugin(void) { return new HmmDecoder(); }
...
HmmDecoder::HmmDecoder(void)
    : DefaultGUIModel("HmmDecoder - BENCHMARK VERSION", ::vars, ::num_vars),
      spike_current(0.0), doSample(false) {
...
void HmmDecoder::execute(void) {
  ...
  // output(0) = ((state_guess_buff.back()<1) ? 0 : 1); //candidate for decoder
  // lag issue -1 is for 0 indexing convention
...
void HmmDecoder::initParameters(void) {
  ...
  */
  ...
  buffi = 0;
  bufflen = 1000; // 300;#default///  3000 is dangerous is tdt room...//

  // [BugFixed] I was tempted to use vector initialization code here, but it was
  // overriding the scope of the vector!
  // vFr.resize(2,0);
  // vTr.resize(2,0);
...
void HmmDecoder::decodeSpkBuffer() {
  ...
  delete[] guessed; // this closes seq which is dynamically allocated inside
                    // viterbi
}
...
void HmmDecoder::restartHMM() {
  ...
  guess_hmm =
      HMMv(nStates, nEvents); // NEW, hardcode a default HMM which DOES NOT
...
void HmmDecoder::customizeGUI(void) {
  ...
  QPushButton *abutton = new QPushButton("Print Params"); // todo deleteme
...
void HmmDecoder::aBttn_event(void) { printStuff(); }
